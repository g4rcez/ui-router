...
(async () => {
  try {
    const render = await Render.Init({
      cacheStrategy: "in-memory",
      fileManager: HttpManager,
      onRebuild: Auth.onRebuild,
    });

    await render.cacheAll();

    const server = fastify({ exposeHeadRoutes: false, onConstructorPoisoning: "error" })
      .register(cookie, { parseOptions: {} } as FastifyCookieOptions)
      .addHook("onRequest", Auth.Middleware)
      .register((srv, _, done) => {
    ...
    const getAsset = (request: Request, response: Response): any => {
    ...
    server.get(`${Html.DependencyPath}/*`, getAsset);
    server.get("/assets/*", getAsset);
